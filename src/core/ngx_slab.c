...
ngx_slab_init(ngx_slab_pool_t *pool)
{
    ...
    if (ngx_slab_max_size == 0) {
            /*
            Ò»¸ö uintptr_t ÀàÐÍµÄ±äÁ¿×÷Îª bitmap À´Ê¹ÓÃÊ±£¬
            ¿ÉÒÔÓÃÀ´¸ú×Ù 8 * sizeof(uintptr_t) ¸ö chunk µÄÊ¹ÓÃ×´Ì¬¡£
            ¡°¸Õ¸ÕºÃ¡± ¿ÉÒÔÓÃÒ»¸ö uintptr_t ¿Õ¼äµÄ bitmap ÄÜ¹»¸ú
            ×ÙÒ»¸ö slab ÖÐËùÓÐµÄ chunk Ê±£¬chunk µÄ´óÐ¡¾ÍÊÇ ngx_slab_exact_size£¬
        ...
        ngx_slab_exact_size = ngx_pagesize / (8 * sizeof(uintptr_t)); ý
	...
	// pages¸öngx_pagesize + sizeof(ngx_slab_page_t)£¬ÄÚ´æÇ°ÃæÎª
	// n*ngx_slab_page_tÊý×é£¬ºóÃæÎªn*ngx_pagesizeµÄÄÚ´æÒ³
    pages = (ngx_uint_t) (size / (ngx_pagesize + sizeof(ngx_slab_page_t)));
    // ³õÊ¼»¯Ç°ÃæµÄngx_slab_page_tÊý×é£¬×îºóÕâÐ©Êý×é¶¼»á
    // ¹Ò½Óµ½slots
...
ngx_slab_alloc_locked(ngx_slab_pool_t *pool, size_t size)
{
    ...
    // ¸Ãslot·ÖÅäµÄchunk´óÐ¡¸ÕºÃ´óÓÚµÈÓÚsize
    // 
    ...
    // ×¢Òâ: 2^shift »òÕß1<<shift±íÊ¾µÄÊÇchunk´óÐ¡

    // ÏÂÃæ´úÂëÖÐ¿´µ½(xxx) << shift±íÊ¾µÄÊÇ(xxx)*chunk_size
    ...
    if (page->next != page) {
        ...
        if (shift < ngx_slab_exact_shift) {
            ...
            do {
                // page - pool->pagesÎªÓÃÓÚ·ÖÅäµÄslabÔÚslabÊý×éµÄÎ»ÖÃ
                // Ò»¸öpageµÄ´óÐ¡Îª1<<ngx_pagesize_shift£¬ÄÇÃ´p¾ÍÊÇ
                // ¶ÔÓ¦µÄÄÚ´æÒ³Ïà¶ÔÆðÊ¼Î»ÖÃµÄÆ«ÒÆ
                p = (page - pool->pages) << ngx_pagesize_shift;//Ïàµ±ÓÚ³ËÒÔÄÚ´æÒ³´óÐ¡
                // bitmapÖ¸Ïò¶ÔÓÃÓÚ·ÖÅäÄÚ´æµÄslab¹ÜÀíµÄÄÚ´æÒ³
                bitmap = (uintptr_t *) (pool->start + p);

                // 1 << (ngx_pagesize_shift - shift) ±íÊ¾¸Ãslot¿ÉÒÔ·ÖÅä¶àÉÙ¸öchunk
                // 1<<shift±íÊ¾¸ÃslotµÄchunk´óÐ¡£¬1<<ngx_pagesize_shift±íÊ¾page´óÐ¡
                // chunkÊýÁ¿Îª2^ngx_pagesize_shift  / 2^shift  = 2^(ngx_pagesize_shift-shift)
                map = (1 << (ngx_pagesize_shift - shift))
                          / (sizeof(uintptr_t) * 8);
                //map±íÊ¾ÐèÒª¶àÉÙ¸öbitmapÀ´Ó³Éä¸ú×ÙËùÓÐµÄchunk
                for (n = 0; n < map; n++) {

                    if (bitmap[n] != NGX_SLAB_BUSY) {
                        //bitmapËùÓÐÎ»ÀïÃæÓÐ0£¬ËµÃ÷ÓÐ¿ÕÓàµÄchunk
                        for (m = 1, i = 0; m; m <<= 1, i++) {
                            if ((bitmap[n] & m)) {
                                continue;
                            }
    ...
    if (page) {
        if (shift < ngx_slab_exact_shift) {
            ...
            // n = bitmap_chunks
            n = (1 << (ngx_pagesize_shift - shift)) / 8 / s; // n±íÊ¾ÐèÒªÓÃÓÚ´æ´¢bitmapµÄchunkÊýÁ¿

...
// 子进程创建的时候ngx_worker_process_cycle中根据此变量创建线程
...
ngx_master_process_cycle(ngx_cycle_t *cycle)
{
    ...
    ngx_start_worker_processes(cycle, ccf->worker_processes,
                               NGX_PROCESS_RESPAWN);
    // 启动cache进程
    // 有些模块需要文件cache，比如fastcgi模块，这些模块会
    // 把文件cache路径添加到cycle->paths中，文件cache管理进程
    // 会定期调用这些模块的文件cache处理钩子处理一下
    // 文件cache,其实一共会启动两个进程，这些进程的
    // detached会被设置为1
